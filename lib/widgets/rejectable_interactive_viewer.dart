/// Fork of [InteractiveViewer] that allows rejecting single-pointer panning.
/// 
/// By default panning over [InteractiveViewer] triggers 
/// [ScaleGestureRecognizer], even if the view is not zoomed. This fork resolves
/// conflicts with other gesture recognizers, e.g. when you want to drag
/// another widget, while the view is not zoomed, preserving the ability to 
/// scale.
...
class VelocitySensitiveScaleRecognizer extends ScaleGestureRecognizer {
  VelocitySensitiveScaleRecognizer({
  ...
  void handleEvent(PointerEvent event) {
    // Reject single-pointer drag gestures when pan is disabled.
    if (!panEnabled && pointerCount == 1 && event is PointerMoveEvent) {
        ...
        gestures: <Type, GestureRecognizerFactory>{
          VelocitySensitiveScaleRecognizer:
              ...
              >(() => VelocitySensitiveScaleRecognizer(
                ...
                VelocitySensitiveScaleRecognizer instance,
              ) {
                instance
...
Offset _exceedsBy(Quad boundary, Quad viewport) {
  ...
  for (final point in viewportPoints) {
    final Vector3 pointInside = CustomInteractiveViewer.getNearestPointInside(
      point,
      boundary,
    );
